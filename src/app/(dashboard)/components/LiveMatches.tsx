// src/app/(dashboard)/components/LiveMatches.tsx
"use client";
...
export default function LiveMatches({
  ...
  const totalPages = Math.ceil(liveMatches.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentData = liveMatches.slice(startIndex, startIndex + itemsPerPage);
            ...
            <Table className="border-collapse min-w-3xl">
              ...
              <TableBody className="bg-white">
                {currentData.map((match, index) => (
                  <TableRow
                    key={`live-${match.teamA.name}-${match.teamB.name}-${
                      startIndex + index
                    }`}
                  >
                    ...
                    <TableCell className="px-6 py-4 min-w-2xs">
